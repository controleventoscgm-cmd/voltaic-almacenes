...
export class IaService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!apiKey) {
      this.logger.warn('IA_API_KEY no está configurada en el .env. El módulo de IA fallará al usarse.');
    }
  ...
  async interpretarTexto(texto: string): Promise<any> {
    ...
    const systemPrompt = `
      Eres un asistente de inventario experto. Tu tarea es extraer información de compras de un texto en lenguaje natural.
    ...
    try {
      ...
      throw new InternalServerErrorException('Error al procesar el texto con la IA. Verifique la configuración.');
    }
  }
}